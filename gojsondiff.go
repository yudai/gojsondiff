...
func emptyJudge(value any) bool {
...
func (differ *Differ) compareArrays(
	...
	for delCandidate := maybeDeleted.Front(); delCandidate != nil; delCandidate = delNext {
		...
		for addCandidate := maybeAdded.Front(); addCandidate != nil; addCandidate = addCandidate.Next() {
			...
			if ok, _ := differ.compareValues(Name("test"), delCan.item, addCan.item); ok {
...
func (differ *Differ) compareValues(
	...
	case float64:
		...
		if ok && compareFloat(leftValue, rightValue) {
			return true, nil
		}
		return false, NewModified(position, left, right)
...
// 比较两个 float 大小，采用两者差值的绝对值小于某个数判断是否相等
// 当浮点数很小时，使用科学计数法中 0 的个数再减 4 表示的数决定是否两个相等
func compareFloat(x, y float64) bool {
	d := math.Abs(x - y)
	exponentX, exponentY := getDecimalExponent(x), getDecimalExponent(y)
	if exponentX == exponentY && exponentX < 0 {
		c := math.Pow(10, float64(exponentX-3))
		return d < c
	}
	return d < 1e-4
}
...
func getDecimalExponent(f float64) int {
	...
	// Split the string on the "e" to separate the mantissa and the exponent.
