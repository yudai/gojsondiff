...
func (differ *Differ) Compare(
	...
	return differ.CompareObjects(leftMap, rightMap, conf), nil
}
...
func (differ *Differ) CompareArrays(
	...
	deltas := differ.compareArrays(leftArr, rightArr, conf)
...
func (differ *Differ) CompareObjects(
	...
	deltas := differ.compareMaps(left, right, conf)
...
func (differ *Differ) compareMaps(
	...
	for _, name := range names {
		if rightValue, ok := right[name]; ok {
			same, delta := differ.compareValues(Name(name), left[name], rightValue, conf)
			if !same {
...
func (differ *Differ) CompareSlices(
	...
	deltas := differ.compareArrays(left, right, conf)
	return &diff{deltas: deltas}
}
...
func (differ *Differ) compareArrays(
	...
	for i := 0; i <= len(lcsPairs); i++ { // not "< len(lcsPairs)"
		...
		if len(delSlice) > 0 && len(addSlice) > 0 {
			...
			bestDeltas, delSlice, addSlice = differ.maximizeSimilarities(delSlice, addSlice, conf)
...
func floatCompare(l, r float64, conf *CompareConfig) bool {
	if conf == nil {
		return l == r
	} else {
		return math.Abs(l - r) < conf.FloatEpsilon
	}
}
...
func (differ *Differ) compareValues(
	...
	case map[string]interface{}:
		...
		childDeltas := differ.compareMaps(l, right.(map[string]interface{}), conf)
	...
	case []interface{}:
		l := left.([]interface{})
		childDeltas := differ.compareArrays(l, right.([]interface{}), conf)
	...
	case float64:
		if !floatCompare(left.(float64), right.(float64), conf) {
			return false, NewModified(position, left, right)
		}

	case float32:
		if !floatCompare(float64(left.(float32)), float64(right.(float32)), conf) {
			return false, NewModified(position, left, right)
		}
...
func (differ *Differ) maximizeSimilarities(
	left []maybe,
	right []maybe,
	conf *CompareConfig,
) (resultDeltas []Delta, freeLeft, freeRight []maybe) {
	...
	for i, leftValue := range left {
		for j, rightValue := range right {
			_, delta := differ.compareValues(Index(rightValue.index), leftValue.item, rightValue.item, conf)
